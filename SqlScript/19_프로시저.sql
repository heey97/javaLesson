...
/*
	...
	( 매개변수이름 [IN 또는 OUT] 데이터 타입,.....) IN : 입력매개변수, OUT : 출력매개변수
	[IS 또는 AS]
		프로시저 변수 선언
	BEGIN
		프로시저 작업 내용 SQL들....
...
WHERE tc.CUSTOM_ID = (
	SELECT tb.CUSTOMID
	FROM TBL_BUY tb
	WHERE quantity = (SELECT max(quantity) FROM TBL_BUY tb2)
);

--프로시저 SQL로 만든 프로그램. => 여러 개의 DML로 구서이 됩니다.
--						 => 필요에 따라 조회 결과를 저장하는 변수를 사용할 수 있따.
...
BEGIN
   ...
   SELECT name,age 
      INTO p_name , p_age   	-- 조회 결과를 출력매개변수에 저장
   FROM "TBL_CUSTOM" tc 
   WHERE CUSTOM_ID =cid;
END;

-- 프로시저 실행 : 출력매개변수가 있는 프로시저
DECLARE
...
CREATE OR REPLACE PROCEDURE money_of_day(
   mcustom_id IN tbl_buy.CUSTOMID %TYPE,
   mday IN varchar2,
   mmoney OUT NUMBER
)
IS
...
BEGIN
   ...
   INTO VPCODE,VCNT
   FROM TBL_BUY
   WHERE CUSTOM_ID = mcustom_id
   AND TO_CHAR(BUY_DATE,'yyyy-mm-dd') = mday;
