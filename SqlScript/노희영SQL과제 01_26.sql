...
SELECT tb.CUSTOMID "아이디", tp.price*tb.QUANTITY "총 구매금액"
...
SELECT tc.NAME "이름 ",tb.PCODE "상품코드",tc.AGE "나이"
...
SELECT tp.PNAME "상품명", tp.PCODE "상품코드", SUM(tp.PRICE*tb.QUANTITY) "팔린 금액"
FROM TBL_PRODUCT tp ,TBL_BUY tb 
WHERE tp.PCODE = tb.PCODE
GROUP BY tp.PCODE,tp.PNAME
