...
public class JavaWordApp_V2 {
    ...
    private void removeWord() {
        ...
        for(int i=0;i<words.size();i++){   //for(JavaWord w : words) - > 오류 (인덱스가 정져있기때문에 인덱스가 변하게되면 오류)
            if(words.get(i).getEnglish().equals(find)){
                ...
                if(System.console().readLine().equals("n"))
                    ...
                    //삭제후 바뀐 인덱스를 다시 현재로 찍어줘야 정상적으로 비교가능
                    i--;
                }            
            }   //단어 비교 if end
        }   //for end
    ...
    private JavaWord searchFirstWord(String english) {
        for(JavaWord word : words) {
            if(word.getEnglish().contentEquals(english)) {
    ...
    private List<JavaWord> searchAllWord(String english){
        ...
        for(JavaWord enword : words) {
            if(enword.getEnglish().matches(".*"+english.trim()+".*")) {
               list.add(enword);          //일치하는 하는 단어를 만날 때마다 저장
            }
        }
        return list;
    }
    //레벨
    private List<JavaWord> searchAllLevel(int level){
        List<JavaWord> list = new ArrayList<>();    //검색 결과 저장할 리스트
        for(JavaWord levword : words) {
            if(levword.getLevel() ==level) {
               list.add(levword);          //일치하는 하는 단어를 만날 때마다 저장
            }
        }
        return list;
    }
    //레벨과 단어
    private List<JavaWord> searchAll(String english, int level){
        List<JavaWord> list = new ArrayList<>();    //검색 결과 저장할 리스트
        for(JavaWord levword : words) {
            if(levword.getLevel() ==level && levword.getEnglish().matches(".*"+english.trim()+".*")) {
               list.add(levword);          //일치하는 하는 단어를 만날 때마다 저장
            }
        }
        return list;
    ...
    private void searchWordBy(){
        System.out.println("\t::단어 검색합니다.(단어 조회는1, 레벨 조회는 2, 단어와 레벨로 조회는)::");
        String finden = null;
        int findlev = 0;
        ...
        switch (System.console().readLine()){ // 키보드가 입력할 변수를 switch문에 담음
            case "1":
                ...
                finden = System.console().readLine();
                list = searchAllWord(finden);
            break;
            case "2":
                System.out.print("검색할 레벨을 입력하세요. >>>>");
                findlev = Integer.parseInt(System.console().readLine());
                list = searchAllLevel(findlev);
                break;
            case "3":
  ...
  /*   private void searchWord() {
        ...
        if(results.size()==0)
                ...
                printWordList(results);        //이쁘게 출력은 나중으로..
