...
-- 2) 테이블을 바꾼 예시 : 회원 'twice'가 '구매'한 '상품명과 가격'을 조회하세요.
...
SELECT CATEGORY , PNAME , PRICE , QUANTITY * PRICE "총 구매 금액" , tb.BUY_DATE  
FROM TBL_PRODUCT tp , TBL_BUY tb 
WHERE tp.PCODE = tb.PCODE 
AND tp.CATEGORY = 'B1';
...
-- 외부 조인 : select * from 테이블1 t1 join 테이블2 t2 on t1.공통컬럼 = t2.공통컬럼;
