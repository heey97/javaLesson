...
public class A27ArrayBasic {
	...
	public static void main(String[] args) {
		...
		System.out.println("ar1 배열의 크기 : " + arr1.length);    //5
		System.out.println("ar1 배열의 크기 : " + arr2.length);    //10
		...
		for(int i =0; i<arr2.length; i++){
			System.out.println("i="+i+",arr1["+i+"]="+arr2[i]);
		}
		...
		System.out.println("arr1배열의 합계는"+sum);
		...
		printArray(arr1); //배열 요소의 값을 출력하는 메소드
		printArray(arr2); 
		
		//메모리에 저장되는 데이터는 주소를 갖습니다.
		//arr1, arr2 배열의 이름은 배열의 시작주소,
		//메소드가 실행될 떄 배열의 시작주소를 메소드의 인자로 전달합니다.	
	...
	public static void printArray(int[] array){
		...
		for(int i=0;i<array.length;i++){
			System.out.print(i+"번째 숫자를 입력해주십시오. : ");
			array[i] = sc.nextInt();
		}
		for(int i=0; i<array.length;i++){
			System.out.println("i="+i+",arr1["+i+"]="+array[i]);
		}
