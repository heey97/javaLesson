...
public class CartApp_4 {
    ...
    private void showMenu() {
        ...
        System.out.println("[B] 구매하기   [D] 구매 취소  [Q] 구매 수량 변경    [T]날짜별 결제 금액 조회  [X] 구매 종료");
        System.out.println("::장바구니::[A] 담기  [L] 목록  [R] 삭제   [Y] 모두 구매 ");
        System.out.println(".".repeat(70));
    }

    private void showMyPage(String customerid) {
        List<CustomBuyVo> result = buyDao.selectBuyList(customerid);
    ...
    public void searchProductListByPname() {
        ...
        String pname = System.console().readLine();
                        List<ProductVo> productlist = productDao.selectByName(pname);
                        for(ProductVo vo : productlist)
                        System.out.println(vo);
    ...
    private void gogo(){
        ...
        priceMap = productDao.getPriceTable();
